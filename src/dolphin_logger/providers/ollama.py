"""
Ollama provider implementation for Dolphin Logger.
Handles Ollama API requests using REST API calls.
"""
...
def handle_ollama_request(
    ...
    """
    Handles requests to Ollama APIs.
    ...
    """
    print(f"DEBUG - Sending Ollama API request: {method} {url}")
    ...
    try:
        ...
        print(f"DEBUG - Ollama API Response status: {api_response.status_code}")
        ...
        if is_stream:
            def generate_ollama_stream_response():
            ...
            resp = Response(stream_with_context(generate_ollama_stream_response()), 
            ...
            try:
                ...
                print("Warning: Ollama API Response was not JSON. Logging raw text.")
        ...
        print(f"ERROR DETAILS (Ollama API - RequestException): Type: {type(e).__name__}, Message: {str(e)}, URL: {url}")
        ...
        print(f"UNEXPECTED OLLAMA API HANDLER ERROR: Type: {type(e).__name__}, Message: {str(e)}")
        resp = jsonify({"error": {"message": "An unexpected error occurred in the Ollama API handler.", "type": type(e).__name__}})
